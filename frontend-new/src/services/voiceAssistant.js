 ...
 * Using Browser Web Speech API directly as per instructions.
 */
...
class VoiceAssistant {
    ...
    /**
     * TTS: Speak text through the browser
     */
    speak(text, onEnd) {
        ...
        utterance.pitch = 0.9; // Lower pitch for more natural female voice
        utterance.rate = 1.0; // Normal human speaking speed
        utterance.volume = 0.9; // Slightly softer for more natural sound
        ...
        utterance.onerror = (event) => {
            // 'canceled' means this utterance was intentionally stopped by a new speak() call
            // Do NOT call onEnd in that case — the new utterance will handle it
            if (event.error === 'canceled' || event.error === 'interrupted') {
                console.log('Speech was replaced by a new utterance (normal behavior).');
                return;
            }
            console.error('Speech error:', event.error);
            if (onEnd) onEnd(); // Continue flow only for genuine errors
        };

        // Start speaking
        try {
            this.synth.speak(utterance);
            console.log(`Speaking: "${text.substring(0, 50)}..." with voice: ${this.selectedVoice?.name}`);
        } catch (error) {
